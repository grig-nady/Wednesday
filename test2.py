...
questions_list = [] 
questions_list.append(Question('КАК ЗОВУТ ДВОРЕЦКО СЕМЕЙКИ АДАМС?','Фрэнк','Ларч'))
questions_list.append(Question('КЕМ ЯВЛЯЕТСЯ БЬЯНКА?','Русалкой','Вампиром'))
questions_list.append(Question('А ЧТО ЕСЛИ Я БОЛГАРКА?','далее','далее'))
questions_list.append(Question('Подача LCL','далее','далее'))
